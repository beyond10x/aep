...
Work moves through the states below and through nothing else. It starts in `inspect`. You may
...
Resolve the candidate version, commit, default branch and repository release policy.

It belongs to phase `preparation`.

From here you may move to:

* **`verify`** — only while `verification.release-published.passed`.
  An already-published release still has to pass the same independent verification.
* **`qualify`** — only while `verification.release-on-default-branch.passed`.
  The candidate commit is reachable from the repository's declared default branch.

### 2. `qualify` — Qualify

Run the repository gate and verify version, changelog and tag provenance inputs.

It belongs to phase `qualification`.

From here you may move to:

* **`await_approval`** — only while `(tests.repository-gate.failed == 0 and verification.release-inputs.passed)`.
  The complete repository gate is green and the release inputs agree.

### 3. `await_approval` — Await release approval

Ask a human to authorize this exact repository, version and commit.
...
* **`publish`** — only while `(approval.release-create.granted and approval.release-create.by_human)`.
  A human approved the exact immutable release subject.

### 4. `publish` — Publish

Create and push the immutable tag using the repository's declared release procedure.
...
* **`verify`** — only while `verification.release-tag-pushed.passed`.
  The remote now holds the requested tag; publication claims are checked next.

### 5. `verify` — Verify

Verify the remote tag, gate run, GitHub Release and policy-declared artifacts.
...
* **`complete`** — only while `verification.source-release.passed`.
  The tag, gate, release record and every declared artifact were verified remotely.

### 6. `complete` — Complete

The source release is published from the default branch and independently verified.
...
**Before entering `await_approval`, the review phase:**
...
**Before entering `publish`, the promotion phase:**
