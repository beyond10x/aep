...
| `templates/` | starting points for a new artifact, read by `aep artifact new`: creating a `story` seeds its body from `templates/story.md`. Still replaceable without leaving the protocol — a tree with no template for a kind gets a one-line body instead of a refusal |
