...
- AEP produces Entity Runtime (ER) definitions directly, as ESS does for services;
- ER decides and records every change;
- Eventlog persists it (`ESS-EVOLUTION.md` steps 3–5).
...
| lifecycle definition | built per command in `kernel.rs`, then thrown away | registered in the store as ER definitions |
...
AEP is the source: definitions are generated from the lifecycle YAML plus the domain schema (unit A1,
extending `kernel.rs`), at every store open, over the kinds each project declares. ESS is a sibling
producer of ER definitions for services, not a source of this model (A9, dropped: § 14).
...
- lowering AEP's planning model from ESS (A9, dropped: § 14);
...
| A9 | dropped | ESS lowers one component's `ServiceIr` into `service/1..3` definitions and refuses `json` fields, `before`/`after` and self-transitions by design (ESS `docs/design/ess-evolution/entity-runtime-lowering.md`: no field lowers to `Json`, the comparison operators, no update becomes a self-transition). AEP's ladders are `kernel/1` definitions generated from adopter-supplied YAML over an open kind set (`aep-backend-eventlog/src/typed.rs:63-77`), and neither repository may depend on the other (invariant 11; ESS `AGENTS.md`). Entity Runtime is the shared foundation; AEP and ESS stay two independent producers of ER definitions |
