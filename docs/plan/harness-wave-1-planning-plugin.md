...
| timestamps | none in the file | git carries authorship and time and cannot be edited by writing a number into a file; a stale `updated:` reads as an observation; and `SEED_AT: Timestamp::EPOCH` (`crates/protocol-cli/src/app.rs:51`) is the standing precedent — a wall clock in diffable output is diff noise. The cost, "how long has this been in draft", is answered by `git log` until P3's journal |
