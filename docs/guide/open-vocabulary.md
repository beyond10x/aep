...
| Blocker type names a blocker kind may carry | `website/docs/reference/vocabulary.md:117` — "decision  review  credential  third-party  capacity  deploy" | open | `crates/aep-domain/src/artifact.rs:408` — the `Other(String)` variant, which a `<type>-blocker` name lands in and resolves to the one `blocker` ladder through its last hyphen segment | — | — | — |
| Test suite names in a tests fact path | `website/docs/reference/vocabulary.md:171` — "unit integration contract property regression mutation fuzz differential e2e smoke" | open | `crates/aep-domain/src/evidence.rs:60` | — | — | — |
