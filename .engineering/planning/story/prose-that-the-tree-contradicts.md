...
**1. `crates/govern/aep-engine/src/load.rs:29` names the wrong row.** The comment above the `drivers` row
...
(`crates/govern/aep-engine/tests/document_tree_order.rs`) and the false one in the source that test
...
| file | kind |
|---|---|
| `crates/drive/aep-driver/tests/shell_echo.rs:125`, `:196` | test doc |
...
**3. `crates/govern/aep-engine/tests/adopting_guide.rs:19-26` is a second hand-maintained copy of the
...
`crates/drive/aep-driver-spec/tests/determinism.rs` is a weaker fork of `crates/drive/aep-driver/tests/determinism.rs`
...
- **Primary surface:** `crates/govern/aep-engine` — cited, two of the three acceptance bullets land there and it holds the only non-comment change
- **Files:** `crates/govern/aep-engine/src/load.rs:28-32` — cited, the `const TREE` comment above the `drivers` row. The false clause is `:29`; `workflows` is `:25` and the row above `drivers` is `artifacts/lifecycles` at `:27`
- **Files:** `crates/govern/aep-engine/tests/adopting_guide.rs:19-27` — cited, `VENDORED` and its doc comment. The story says `:19-26`; the array closes at `:27`
- **Files:** `crates/drive/aep-driver/tests/shell_echo.rs:125`, `:196` — cited, both still uncorrected
...
- **Symbols:** `TREE` (`crates/govern/aep-engine/src/load.rs:22`), `VENDORED` (`crates/govern/aep-engine/tests/adopting_guide.rs:20`), `command.execute` (`profiles/development-driven.yaml:78`, the grant that falsifies the claim) — cited
...
3. **`profiles/development-driven.yaml:73` and `:54-62` — measured.** The `decide_tool` clause is
   ...
   shown false.
4. **`crates/govern/aep-engine/tests/documents.rs:361` — measured.** `DENIALS` misses **6 of 6** phrasings
   ...
   not four sentences.
5. **`crates/govern/aep-engine/tests/documents.rs:297` — measured.** `examples/` is in `READ_BY_PROGRAMS`
   ...
   and `crates/edge/aep-schema/tests/published.rs:66`. Name those.
