...
- `task install` (`cargo install --path crates/edge/aep-cli --locked --force`) and `AGENTS.md`
...
- Every `artifact` verb compares its own version to the newest `protocol:` in the journal; an older
  binary refuses, naming both versions and `cargo install --path crates/edge/aep-cli --locked --force`.
