...
status: cleared
title: Qualify ER recorded Eventlog execution before planning-store migration
owner: Astra
relations:
...
story:eventlog-planning-authority-migration requires the qualified asynchronous ER Eventlog adapter
and explicit synchronous compatibility bridge over the verified provider foundation. The first ER
contract/reference executor is currently being implemented; the adapter/bridge and remaining
Eventlog atomic-group/tamper/canonical-byte qualification are not complete.

## Resolution

Close this dependency only against exact accepted source commits and actual provider/replay/receipt
test evidence, with PostgreSQL selected and the runtime compiler minimum qualified. Mere existence
of old synchronous providers, a design approval or a library compile does not establish readiness.
No user decision is missing; Astra continues the already approved provider/ER sequence.

## References

Atlas ADR0050; approved ESS evolution revision1; entity-runtime story async-recorded-contract-executor;
local-evidence:ess-evolution/waves/0002-er-contract/README.md and waves/0003-eventlog-proof/README.md.
