...
| `aep plan store export --engineering <dir> --into <dir> --map <file>` | copies an `aep.project/2` store into a new tree store once, for the cutover. The source is read and never written. Every artifact becomes its kind's typed entity under an identity derived from its locator, with its history as anchor evidence; every home path becomes `workspace:<path>` or `home-path:sha256:<digest>`. The copy is compared artifact by artifact with the source before it is reported, and the map of old identities and home paths goes to `--map`, which must be outside every repository |
