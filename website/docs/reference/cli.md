...
| `aep plan artifact evidence <id> --from <report.json> --suite <suite.json> [--ref <reference>]` | reads standalone `ess-conformance-report/2` with the exact original suite/1–4 JSON. Checks all category counts, original-byte identity and final outcomes; records diagnostic history with both input references and the exact completion millisecond. Legacy coverage remains unknown and cannot qualify complete conformance. `--suite` requires report/2; report/1 keeps its existing behavior. The event backend must represent the date, so planning explicitly refuses times after `9999-12-31T23:59:59.999Z` before opening a store; the standalone reader and typed evidence retain the full unsigned 64-bit range |
| `aep plan artifact evidence <id> --from <report.json> --suite-input <input.json> [--ref <reference>]` | admits coverage report/2 against `ess-conformance-input/1`, which carries the original suite/5 string and its complete original parent chain. Records descriptive counts, statuses, selection and refusal occurrences. `--suite-input` and `--suite` are mutually exclusive; either requires `--from`. Raw unfiltered suite/5 also works with `--suite`; a filtered suite requires the input document and all parents. Detailed run/2 and report/1 pairing are refused. The same planning calendar limit applies |
...
| `aep plan artifact validate [--strict] [--outcome-within <days>] [--against <revision>]` | every file, every edge, every status, accumulated into one list: a file where its id does not put it, an edge pointing at nothing, a cycle, a duplicate id, a status the lifecycle does not have. Over a markdown plan it reads the event log too: a document edited outside a command is **drift**, and a `revision:` higher than any event for it records is a **forged revision** — a number no write produced, reported and never refused. It also lists every story past its ladder's first rung and short of its end that declares no `scope` — reported, never failed on, and deliberately not a `--strict` class either: refusing would turn every unscoped story red on the day it landed, which is how a check gets muted. Five classes are reported and not failed on — a status closed on an assertion, a document predating the event log, drift, a `review-result` whose body states its findings as prose only, and a `review-result` at least `--outcome-within` days old (14 by default) that no `review_outcome` record names — because each is legal in a store people are working in; `--strict` turns each into exit 1 for the caller that wants it, prints the same lines and names which class decided. On an Eventlog plan two of the five are not computed yet — a status closed on an assertion and a document predating the event log both read the journal, which a migration freezes under the projection — and the review-age class is read from the authority for the `review-result` documents and the artifacts they `reviews` only; the faithful port of all three is `story:strict-advisory-classes-on-an-eventlog-plan`. A `findings` block that is *malformed* is a problem rather than a report: `new` refuses one, so a broken block in the store was written past the command. The review age is the one line of this verb that is a function of when it was run, read against the clock at the edge |
| `aep plan artifact resolve <id> [--first <digest>] [--onto <revision>]` | on an `aep.project/3` tree store, joins an artifact that two merged branches both changed. After `git merge` such an artifact has two heads and refuses every ordinary write; `resolve` records one merge decision over every head, keeping the content of `--first`, or of the one head whose files `--onto` (default `origin/main`) holds. Nothing is deleted: the other branch's decisions stay in the history and are printed as `not carried over`, so what is still wanted can be issued again. The projection is re-rendered, which clears the artifact's `.md` conflict. On a tree store, `validate` always adds the rules only a tree has, and `--against <revision>` holds its files to that revision: its files verify (V1–V5, also against `--against <revision>`; when that revision holds no tree store — an `aep.project/2` base, as a cutover is measured against — V2 is skipped and the run prints `V2 skipped: <revision> holds no tree store (<path>/store.json)`, while a revision Git cannot read is still refused), no artifact has forked (S3), every `.md` is its artifact's render (S5) and no committed file carries a home path (S9) |
| `aep plan artifact render` | rewrites the Markdown projection from the store, the way a write would. On a tree store it is how a projection is rebuilt after a merge or an export, and it writes every document as `format: aep.planning-md/2`; an `aep.project/2` store keeps writing `aep.planning-md/1`. `validate` holds each document to the render of its own `format:` tag (S5), so a tree projection still in `/1` validates until it is re-rendered, and the first write re-renders all of it |
...
| `aep plan store init-tree --engineering <dir> --scope <scope> [--tenant planning] [--protocol adp/1] [--profile development.standard] [--protocols ..]` | creates a planning store of typed Entity Runtime entities on an `eventlog-tree` store, which version control merges, and the `aep.project/3` selector that names it. Refuses a project that already has a selector or a non-empty `state/`: a new store is created, never written over an existing one |
| `aep plan store export --engineering <dir> --into <dir> --map <file> [--fixup <file>]` | copies an `aep.project/2` store into a new tree store once, for the cutover. The source is read and never written. Every artifact becomes its kind's typed entity under an identity derived from its locator, with its history as anchor evidence; every home path becomes `workspace:<path>` or `home-path:sha256:<digest>`. The copy is compared artifact by artifact with the source before it is reported, and the map of old identities and home paths goes to `--map`, which must be outside every repository. `--fixup` names a JSON object of literal replacements applied to every string, such as a name that may not enter public history; the map records them |
| `aep plan store install-hooks [--repository <path>]` | writes the repository's `hooks/pre-push`: a push whose unpushed commits touch `.engineering/` runs `aep plan artifact validate` first, and is refused when the installed `aep` is not the version that installed the hook or the pushed commit is not the clean checked-out tree. A hook it did not write is never replaced. When a hook manager owns `core.hooksPath` it says so, and that manager's install (Gates: `b10x-gates install`) chains the hook |
...
| `aep observe evidence inspect <files>… [--at …] [--horizon 7d]` | reads the evidence document `aep govern evaluate --evidence` submits and reports, per record, when somebody last looked. For `ess_conformance_v2`, it re-admits the embedded original report/suite pair; JSON output retains the exact completion millisecond as decimal `completed_at` |
...
The opt-in `development.ess-conformance-v2` profile uses `adp-ess-conformance/1` and the `ess-conformance-v2` principle. It extends `development.standard`; existing defaults stay unchanged. Its typed evidence is an `ess_conformance_v2` record carrying the original `report_json` and `suite_json` strings. Admission checks the source pair again on submission and snapshot restoration. Raw deserialization confers no admission. Independently authored `constraints.ess_conformance_v2` names the expected model, exact suite reference and selected scenario IDs; evidence does not fill those expectations. Suite/1–4 has unknown coverage, so count-stage diagnostics cannot complete ESS-governed work under this profile. Planning history remains descriptive and is not a typed evidence archive.

The separate opt-in `development.ess-conformance-coverage` profile uses `adp-ess-conformance-coverage/1` and the `ess-conformance-coverage` principle, extending `development.standard`. Its `ess_conformance_coverage_v1` evidence carries original `report_json` and `suite_input_json` strings. The input has exactly `format: "ess-conformance-input/1"`, `suite_json` and `parent_suites`; parents are original suite/5 strings in nearest-parent-first order, ending at an unfiltered suite. An unfiltered selected suite has no parents. Admission checks exact inner UTF-8 bytes, full inventory, source ownership, retained refusal occurrences and each filtering relationship. Parent files are never resolved from the filesystem or network.

`govern workflow instruct` currently renders all registry principles whose obligations reach the workflow's phases; it does not resolve a task profile. Its generated instructions therefore list the coverage principle beside other available principles. The principle's summary names its opt-in context. Runtime selection follows the task's resolved profile, and existing default profiles do not acquire this requirement.

Independently authored `constraints.ess_conformance_coverage_v1` contains `model`, `suite`, the full `selection` (scope, origins, filter and parent reference), and sorted `selected_ids`. The task also names its subject. A complete, nonempty, all-pass selection qualifies only against those expectations, the named current model digest, an independent `conformance-runner`, and a matching observation instant that is not in the future. An explicitly declared requirement horizon applies; there is no default expiry. Unknown coverage, in-scope refusals, empty selections and inconclusive execution remain descriptive; failed execution contradicts the requirement. Counts and diagnostic facts alone cannot establish qualification.

`govern evaluate`, `observe evidence inspect`, and driver start/status/resume all install the optional coverage reader and re-admit typed JSON/YAML source strings. Inspection exposes exact decimal `completed_at`. Engine consumers explicitly install `CoverageReader`; configurations without it refuse coverage submissions and restoration. Raw deserialization and snapshot bytes carry no admitted cache. Count and coverage readers can coexist, and report/1 and count-stage APIs retain their existing behavior. Planning history is descriptive and cannot substitute for a typed record.
