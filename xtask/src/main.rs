...
/// Where the projection of [`OBSERVATION_SPEC`] against [`OBSERVATION_FIXTURE`] is committed.
///
/// A *tree* rather than a file, so this one output gets the orphan scan the other three do not
/// need: a patch file for an object nothing patches any more would sit in the repository looking
/// like a proposal somebody still stands behind, and a check that only compares what is generated
/// never sees it. The tree is committed for the reason the simulation is, one step further on —
/// a rule that starts writing a patch where it used to write an obligation moves exactly these
/// bytes, and that is the change a reviewer has to be shown rather than told about.
const OBSERVATION_PROJECTION: &str = "examples/k3d-dev-cluster/projection";
...
enum Command {
    ...
    /// Regenerate the committed infrastructure IR, simulation, drift and projection of the
    /// example observation.
    Infra {
...
fn sync(
    ...
    if check {
        ...
        if !orphaned.is_empty() {
            let _ = writeln!(
                detail,
                "{} {noun} are generated by nothing any more: {}",
                orphaned.len(),
                orphaned.join(", ")
            );
        }
...
/// Writes or checks the four committed outputs derived from the example observation.
///
/// Three files and one tree. The bytes come from `protocol infra compile|simulate|diff|project
/// --format json`, each of which prints exactly what its own `--out` persists — one producer, so
/// this check can never disagree with the CLI about what a document looks like. Byte comparison,
/// not semantic: every one of them is deterministic by construction, so any drift is a real change
/// in a model, an evaluator or the fixture, and each of those must arrive as a reviewed diff.
fn infra(root: &Path, check: bool) -> Result<()> {
    ...
    // The fourth output is a tree, so it goes through `sync` — the same writer, orphan scan and
    // prune the projections and the suites use. Reaching it through the command line rather than
    // by linking `infra-project` is the rule [`projections`] states: what gets committed has to be
    // what the command a person runs produces.
    let printed = protocol_stdout(
        &[
            "infra", "project", "--spec", &spec, "--path", &fixture, "--format", "json",
        ],
        "projecting the example specification",
    )?;
    let report: serde_json::Value = serde_json::from_slice(&printed)
        .context("reading what `protocol infra project --format json` printed")?;
    let mut expected = BTreeMap::new();
    for artifact in array(&report, "artifacts")? {
        expected.insert(text(artifact, "path")?, text(artifact, "contents")?);
    }
    sync(
        &root.join(OBSERVATION_PROJECTION),
        &expected,
        check,
        &[],
        "projection files",
        "the specification and the observation",
        "cargo xtask infra",
    )
}
