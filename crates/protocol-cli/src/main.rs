...
enum InfraCommand {
    ...
    /// Evaluate a desired-state specification against an observed cluster.
    ///
    /// Every expectation gets a three-valued verdict: `ok` when the snapshot satisfies it,
    /// `gap` when it contradicts it — carrying what would have to change — and `unk` when the
    /// snapshot cannot decide, carrying the reason it cannot. `unk` is never `gap`: an
    /// expectation nobody can evaluate has not failed.
    ///
    /// A simulation is a report, not a gate, exactly as `diagnose` is: exit 0 whatever the
    /// verdicts say. Exit 1 means the *input* was unusable — a refused specification, a refused
    /// bundle, a tampered IR document.
    Simulate {
        /// The desired-state specification, an `infra-spec/1` document in YAML or JSON.
        #[arg(long)]
        spec: PathBuf,
        /// A bundle or a persisted IR document to evaluate it against.
        #[arg(long)]
        path: PathBuf,
        /// How to render the report.
        #[arg(long, value_enum, default_value_t = DiagnoseFormat::Text)]
    ...
    /// probes, environment, selectors, routing, configuration digests, claim phases and
    /// references that broke or healed. Pods are deliberately absent: they are renamed on every
    /// rollout, and a report listing a thousand of them is one nobody reads.
    ///
    /// Refuses one thing: two snapshots scanned in different kubeconfig contexts. Comparing two
    /// clusters produces a change list where everything was added and everything was removed.
    Diff {
        /// The snapshot compared from — a bundle or a persisted IR document.
        #[arg(long)]
        from: PathBuf,
...
/// Reads a desired-state specification through its validation.
///
/// YAML, and JSON by consequence — `serde_yaml` reads both, so a specification generated by a
/// tool needs no second reader. A document that does not deserialize at all is one refusal
/// (`INFRA-SPEC-002`) rather than a serde sentence, so a harness matches on a code here exactly
/// as it does on a bundle.
fn infra_spec_at(
    path: &Path,
) -> Result<Result<infra_spec::InfraSpec, infra_domain::ValidationErrors>> {
    let text =
        std::fs::read_to_string(path).with_context(|| format!("reading {}", path.display()))?;
    Ok(infra_spec::read_spec(&text))
}

/// `protocol infra simulate`
///
/// Exit 0 whatever the verdicts say: a simulation is a report, not a gate — the argument IW2
/// made for `diagnose`, and the same one. Exit 1 is reserved for an input that could not be
/// simulated at all.
fn infra_simulate(spec_path: &Path, path: &Path, format: DiagnoseFormat) -> Result<ExitCode> {
    ...
    match format {
        DiagnoseFormat::Text => out!("{}", infra_spec::simulation_to_text(&simulation)),
...
/// `protocol infra diff`
///
/// Exit 0 for any comparison that ran, including one that found a hundred changes: drift is a
/// report too. Exit 1 for an unusable input, and for the one refusal — two snapshots of
/// different clusters.
fn infra_diff(from: &Path, to: &Path, format: DiagnoseFormat) -> Result<ExitCode> {
    let mut sides = Vec::with_capacity(2);
    for path in [from, to] {
        match infra_ir_at(path)? {
            InfraIrLoaded::Ir(ir) => sides.push(ir),
            InfraIrLoaded::Refused(errors) => {
                match format {
                    DiagnoseFormat::Text => infra_refusals(&errors),
                    DiagnoseFormat::Json => print_serialised(&errors, Format::Json)?,
    ...
    let report = match infra_spec::drift(&sides[0], &sides[1]) {
        Ok(report) => report,
        Err(refusal) => {
            match format {
                DiagnoseFormat::Text => eprintln!("error: {refusal}"),
                DiagnoseFormat::Json => print_serialised(&refusal, Format::Json)?,
            }
            return Ok(exit_code(false));
        }
    };
    match format {
        DiagnoseFormat::Text => out!("{}", infra_spec::drift_to_text(&report)),
        DiagnoseFormat::Json => out!("{}", report.to_json()),
    }
    Ok(ExitCode::SUCCESS)
}
