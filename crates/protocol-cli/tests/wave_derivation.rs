...
fn scope_add_writes_a_typed_entry_and_bumps_the_revision_once() {
    ...
    assert!(
        document.contains("scope:") && document.contains("crates/aep-domain/src/artifact.rs"),
        "{document}"
    );
    assert!(
...
fn show_prints_the_scope_and_json_carries_it_as_an_array() {
    ...
    assert!(
        text.contains("scope") && text.contains("crates/aep-domain/src/artifact.rs"),
        "{text}"
    );
    assert!(
    ...
    assert_eq!(scope[0]["path"], "crates/aep-domain/src/artifact.rs");
    assert_eq!(scope[0]["confidence"], "cited");
    assert_eq!(scope[1]["confidence"], "inferred");
}
