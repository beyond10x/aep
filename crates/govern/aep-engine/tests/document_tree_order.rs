...
//! `crates/govern/aep-engine/tests/evidence_scan.rs:29-35` is this crate's precedent for taking it.
...
fn tree_directories() -> Vec<String> {
    let source = fs::read_to_string(repository_root().join("crates/edge/aep-project/src/load.rs"))
