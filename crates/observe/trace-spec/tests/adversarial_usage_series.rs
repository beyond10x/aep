...
///
/// `ExpectationKind::UsageTrend`, `crates/observe/trace-domain/src/spec.rs`: *"`cache.read_tokens` says
...
/// from a fixed seed, and it holds. `range_of` (`crates/observe/trace-domain/src/raw.rs:2193`) already
...
/// (`crates/observe/trace-spec/src/report.rs:254`) justifies itself with *"a pair of equal values is
...
///
/// That is the whole argument `Trend::moves` (`crates/observe/trace-domain/src/spec.rs:245`) and
...
/// read from.
///
/// `TraceIr::events_of_request` (`crates/observe/trace-domain/src/ir.rs:927`) has two paths and both
...
///
/// `share_of_a_total` (`crates/observe/trace-domain/src/raw.rs:2226`) refuses a share bound outside
...
///
/// `series` (`crates/observe/trace-spec/src/check.rs:1516`) argues for the spelling on the ground that
