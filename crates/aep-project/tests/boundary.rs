...
fn the_engine_normal_graph_has_no_schema_driver_or_acquisition_dependency() {
    let root = workspace();
    let manifest = std::fs::read_to_string(root.join("crates/aep-engine/Cargo.toml"))
    ...
    let library = std::fs::read_to_string(root.join("crates/aep-engine/src/lib.rs"))
        .expect("the engine root is readable");
    assert!(!library.contains("pub mod load"));
    assert!(!library.contains("pub mod project"));
    assert!(!root.join("crates/aep-engine/src/load.rs").exists());
    assert!(!root.join("crates/aep-engine/src/project.rs").exists());
}
