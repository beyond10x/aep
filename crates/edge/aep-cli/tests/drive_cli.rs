...
fn the_cargo_map_starts_a_feature_run_without_the_evidence_gap_flag() {
    ...
    let stopped_for_the_machine = said.contains("METAHARNESS_LIVE=1")
        ...
        || said.contains("use `metaharness aep drive run`");
    assert!(
...
fn a_map_that_is_both_uncoverable_and_unspawnable_reports_the_defect_that_travels() {
    ...
    assert!(
        ...
        "the standalone host refuses model execution after coverage:\n{complaint}"
    );
    assert!(
        !fixture.runs().join("DRIVE-1").exists(),
...
/// A model map cannot turn either neutral entry point into an execution host.
#[test]
fn model_start_and_resume_refuse_without_changing_the_project() {
    let fixture = Fixture::new("model-boundary", false);
    fixture.drive(&["run"], &["--max-iterations", "0"]);
    let map = fixture.directory.join("steps.yaml");
    let source = std::fs::read_to_string(&map).expect("map");
    write(
        &map,
        &format!("{source}  review:\n    steps:\n      - kind: llm\n        prompt: review\n"),
    );
    let before = tree(&fixture.directory);
    for verb in [vec!["run"], vec!["resume", "DRIVE-1/1"]] {
        let output = fixture.drive(&verb, &[]);
        assert_eq!(code(&output), 1, "{}", said(&output));
        assert!(
            stderr(&output).contains("use `metaharness aep drive run`"),
            "{}",
            said(&output)
        );
        assert_eq!(
            tree(&fixture.directory),
            before,
            "a refusal must not mutate the run or project"
        );
    }
}
...
/// `cargo test -p aep-cli -p aep-driver` stayed green in `tests/driving.rs` (14 passed,
