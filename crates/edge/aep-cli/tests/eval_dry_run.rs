...
//! (`crates/edge/aep-cli/fixtures/eval-run/README.md` says exactly how): they are structurally
...
const REPLAYS: [Replay; 4] = [
    ...
    Replay {
        ...
        arm: "driven",
        harness: "claude",
        stream: "crates/edge/aep-cli/fixtures/eval-run/claude-driven-attested.jsonl",
    },
    ...
    Replay {
        ...
        arm: "plugin",
        harness: "codex",
        stream: "crates/edge/aep-cli/fixtures/eval-run/codex-plugin-attested.jsonl",
    },
];
...
fn the_digest_every_manifest_carries_comes_from_the_instruments_row() {
    ...
    let codex_stream = std::fs::read_to_string(
        root().join("crates/edge/aep-cli/fixtures/eval-run/codex-plugin-attested.jsonl"),
    )
