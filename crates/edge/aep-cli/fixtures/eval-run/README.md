...
`crates/edge/aep-cli/tests/eval_dry_run.rs` assembles from them. Nothing here was recorded from a
...
sentence, as `crates/edge/aep-cli/fixtures/metaharness-frame-canonical.json` and the frame contract
...
The two plugin digests are the ones `crates/edge/aep-cli/fixtures/eval-matrix/runs/` already uses for
