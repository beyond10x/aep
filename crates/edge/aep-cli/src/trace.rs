...
///
/// # `protocol` is the canonical spelling, whichever binary ran
///
/// The tool is named `protocol` here even when the process on disk is `aep`, and that is the
/// contract rather than an oversight. Invariant 10 is that the two names are one interface —
/// identical output bytes and exit status — and this record is an *output*: a `command` that
/// followed the caller's binary would make the same check through the two names produce two
/// different documents, so a record could no longer be regenerated by a reader who has the other
/// name installed, and every evidence file already committed here (and every one the guides quote
/// as printed) would diff against a rerun of the command that wrote it.
///
/// This says what was asked for, in the vocabulary of the verb, and the verb's canonical vocabulary
/// spells the tool `protocol`. *Which binary was invoked* is a fact about the operator's `PATH`,
/// not about the observation, and no reader of the record needs it.
///
/// `the_provenance_command_is_the_canonical_spelling_whichever_binary_minted_the_record` in
/// `crates/edge/aep-cli/tests/trace_cli.rs` is the guard for this verb, and
/// `every_evidence_producer_writes_the_same_record_through_either_binary_name` in
/// `tests/evidence_producers.rs` is the guard for the other four that mint a record this way —
/// `specification.rs`, `contract.rs` and `property.rs`'s own `invocation`, and `app.rs`'s
/// `protocol validate`. The documented contract is `website/docs/concepts/evidence.md`.
