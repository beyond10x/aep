...
/// Every `.rs` file under `crates/<area>/<crate>/src`, repository-relative, in a stable order.
...
fn the_alias_scan_finds_the_attributes_it_is_supposed_to_find() {
    // The inverse guard, on the pattern of `crates/govern/aep-domain/tests/invariants.rs`: a scan that has
...
fn every_published_schema_accepts_every_spelling_its_parser_does() {
    ...
    assert!(
        ...
        "the published schemas refuse spellings the parsers read. Add each to \
         `WIRE_ALIASES` in crates/edge/aep-schema/src/alias.rs:\n  - {}",
        missing.join("\n  - ")
    );
}
