...
`crates/edge/protocol-cli/tests/eval_dry_run.rs` assembles from them. Nothing here was recorded from a
...
sentence, as `crates/edge/protocol-cli/fixtures/metaharness-frame-canonical.json` and the frame contract
...
The two plugin digests are the ones `crates/edge/protocol-cli/fixtures/eval-matrix/runs/` already uses for
...
on the same reasoning `crates/observe/trace-spec/tests/fixtures/`' module documentation gives at length.
...
protocol eval matrix <dir> --format json > crates/edge/protocol-cli/fixtures/eval-run/dry-run.matrix.json
protocol eval matrix <dir>                > crates/edge/protocol-cli/fixtures/eval-run/dry-run.matrix.txt
```

once per run, in the order `REPLAYS` declares in `crates/edge/protocol-cli/tests/eval_dry_run.rs`.
