...
//! are under `crates/edge/protocol-cli/fixtures/eval-run/`, derived from them — the derivation and what
...
const ATTESTED_STREAM: &str =
    "crates/edge/protocol-cli/fixtures/eval-run/claude-plugin-attested.jsonl";

/// The same, for Codex — and its terminal event prices nothing.
const CODEX_STREAM: &str = "crates/edge/protocol-cli/fixtures/eval-run/codex-plugin-attested.jsonl";
...
fn a_run_of_arm_driven_is_read_even_though_it_is_not_launched_here() {
    ...
    let ingested = ingest(
        ...
        "driven",
        "claude",
        "crates/edge/protocol-cli/fixtures/eval-run/claude-driven-attested.jsonl",
    );
    assert_eq!(code(&ingested), 0, "{}", stderr(&ingested));
