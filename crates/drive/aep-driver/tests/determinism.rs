...
/// `crates/drive/aep-driver/ambient.rs` and not the same `PathBuf`, so the scan would report it twice and
...
///
/// Reproduced end to end rather than argued: one empty `crates/drive/aep-driver/src/fixtures/protocol.json`
